...
### Git & Commits
- **Conventional Commits:** `fix(scope)`, `refactor(scope)`, `feat(scope)` obrigatório
- **Atomic commits:** Um conceito = um commit (segurança + refactoring = 2 commits)
- **Sem secrets no history:** Revisar `git diff --cached` antes de commit
- `.gitignore` segue padrão Python — manter atualizado

---

## Checklist para Modificações

- [ ] Mudança segue tipo Conventional Commit?
- [ ] Upload validado (MIME, extensão, tamanho)?
- [ ] Inferência pesada em thread pool?
- [ ] Erros genéricos ao cliente, detalhes só em log?
- [ ] DRY aplicado (helpers extraídos)?
- [ ] Type hints presentes?
- [ ] `.gitignore` mantém cobertura?
- [ ] Nenhum secret em git diff?
